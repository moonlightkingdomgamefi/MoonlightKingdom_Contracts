...
contract GuardianGeneralEgg is ERC721Enumerable, Ownable {
    ...
    uint256 public eggcost = 50 ether;
    uint256 public hatchcost = 1 ether;
    uint256 public eggMaxSupply = 1900;
    ...
    event NewEgg(address indexed owner, uint256 id,uint8 rarity);
    ...
    function mint(address _ref) public payable { 
        ...
        if(_ref != devWalletAddress && _ref != msg.sender){
            ...
            if(roll < 15) {
                    grade = 3; // 3-Star at 15%
            }
            else if(roll < 50) { 
                    grade = 2; // 2-Star at 35% 
            }
            else { 
                    grade = 1; // 1-Star at 50%
            }
            Egg memory newEgg = Egg(supply+1,grade);
            eggs.push(newEgg);
            _safeMint(msg.sender, supply+1);
            emit NewEgg(msg.sender, supply+1,grade);
    ...
    function hatch(uint256 _eId) public payable { 
        ...
        Egg storage egg = eggs[_eId];
        if(egg.rarity == 3) {
            games.mintCharacter(msg.sender,3);
        }
        else if(egg.rarity == 2) {
            games.mintCharacter(msg.sender,2);
        }
        else { 
            games.mintCharacter(msg.sender,1);
        }
        _burn(_eId);
